...
class TileGrid extends JPanel{
	...
	public void fillBoard()
    {
    	...
    	for(int i = 0; i < tiles.length; i++)
    	{
    	    ...
    	    masterTiles[i].setPreferredSize(TILESIZE);
            //masterTiles[i].setIcon(img.getImage(i));
    	    masterTiles[i].setIdentifier("Tile_" + (i + 1));
            masterTiles[i].setText(("Tile_" + (i + 1)));
            //masterTiles[i].setIdentifier("Tile_" + (i + 1));
            //masterTiles[i].setText("Tile_" + (randSpot[i] + 1 ));
            
    		tiles[i] = new Tile();
    		tiles[i].setPreferredSize(TILESIZE);
    		tiles[i].setText("Tile_" + (randSpot[i] + 1 ));
    		//tiles[i].setIcon(img.getImage(randSpot[i]));
    		tiles[i].setIdentifier("Tile_" + (randSpot[i] + 1 ));
    		
    		resetTiles[i] = new Tile();
    		resetTiles[i].setPreferredSize(TILESIZE);
    		resetTiles[i].setText("Tile_" + (randSpot[i] + 1 ));
